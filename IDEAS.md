...
*   ~~**Network (Input Gate / IR)**: The client side uses OpenSiv3D `SimpleHTTP`, but it points to dummy endpoints or uses mock data if `ConfigIni` URLs are blank. **Idea:** Build a simple lightweight Node.js/Express or Python/FastAPI companion backend repo to serve `ranking.json`, `list.json`, and handle ZIP distribution.~~ -> **DONE**. Added `mock_input_gate_server.py` as an SQLite persistent local server and hooked up client JSON.
