...
Co::Task<void> InputGateScene::start()
{
	...
	while (true)
	{
		...
		if (m_showUpdateDialog)
		{
			...
			{
				if (KeyConfig::Down(KeyConfig::kButtonStart) && !m_updateInfo.downloadUrl.isEmpty())
				{
					// Open browser
		...
		// Navigation
		if (KeyConfig::Down(KeyConfig::kButtonBack))
		{
			break;
		}

		// Tab Switching (FX-L / FX-R)
		if (KeyConfig::Down(KeyConfig::kButtonFX_L) || KeyConfig::Down(KeyConfig::kButtonFX_R))
		{
